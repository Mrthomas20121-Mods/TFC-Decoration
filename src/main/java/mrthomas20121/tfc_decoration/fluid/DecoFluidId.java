...
public record DecoFluidId(String name, OptionalInt color, Supplier<? extends Fluid> fluid) {
    ...
    private static final List<DecoFluidId> VALUES = Arrays.stream(DecoDyeColor.values()).map(dye -> fromEnum(dye, TFCDecoFluids.dyeColorToInt(dye), dye.getSerializedName() + "_dye", TFCDecoFluids.COLORED_FLUIDS.get(dye).source())).toList();
