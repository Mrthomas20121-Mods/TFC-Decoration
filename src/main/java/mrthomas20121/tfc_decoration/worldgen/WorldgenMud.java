...
public class WorldgenMud implements IWorldGenerator {
    ...
    {
        if (!(BlocksTFC.isWater(world.getBlockState(start)))) return;
        Rock rock = ChunkDataTFC.getRockHeight(world, start);
        final int size = rng.nextInt(10) == 0 ? 3 : 2;
        for (int x = -size; x <= size; x++)
        {
            for (int z = -size; z <= size; z++)
            {
                for (int y = -2; y <= 2; y++)
                {
                    ...
                    world.setBlockState(start.add(x, y, z), BlockDecoration.get(rock, DecorationType.RAW_MUD).getDefaultState());
                }
            }
        }
    }
